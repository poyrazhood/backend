...
3. ${requiredQA.length > 0 ? `İLK ${requiredQA.length} SORU ZORUNLU — bu soruları mutlaka üret:\n   ${requiredQA.map((q,i) => `${i+1}. "${q}"`).join('\n   ')}\n   Kalan sorular şu konulardan seçilsin:` : 'Soru kategorileri şunlardan seçilsin (düz metin, parantez olmadan):'}
   ${hints}
