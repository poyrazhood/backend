...
async function scrapeOne(page, business) {
  ...
  // Geçiş 1: En faydalı (varsayılan) — scroll ile tüm yorumları yükle
  ...
  if (await sortBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    if (await newestOpt.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      // Feed DOM'dan kalkana kadar bekle, sonra yenisini bekle
      // (direkt waitForSelector eskiyi bulur — scroll çalışmaz)
