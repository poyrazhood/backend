/**
 * fix-queue.cjs
 * 1. UNIQUE constraint'i kaldırır
 * 2. Eksik jobları ekler
 *
 * Kullanım:
 *   node scraper/fix-queue.cjs          ← preview
 *   node scraper/fix-queue.cjs --commit ← uygula
 */
...
const KATEGORI_ARAMALAR = {
  'yeme-icme':       ['Kafeler','Restoranlar','Barlar & Gece Hayatı','Fast Food & Paket Servis','Pastane & Fırın','Kahvaltı Salonu','Kahve & Çay Evi'],
  'saglik-medikal':  ['Hastane','Eczane','Klinik & Poliklinik','Diş Hekimi','Spor & Fitness','Psikolog & Psikiyatrist','Göz Doktoru & Optik','Fizyoterapi'],
  'guzellik-bakim':  ['Kuaför & Berber','Güzellik Merkezi','Spa & Masaj','Dövme Stüdyosu','Tırnak Bakımı','Epilasyon & Güzellik'],
  'alisveris':       ['Market & Süpermarket','Alışveriş Merkezi','Elektronik Mağaza','Giyim Mağazası','Kitabevi & Kırtasiye','Mobilya Mağazası','Çiçekçi','Spor Malzemeleri'],
  'hizmetler':       ['Avukat & Hukuk Bürosu','Mali Müşavir & Muhasebe','Nakliyat','Oto Servis & Tamirci','Temizlik Şirketi','Tadilat & Boya','Fotoğrafçı','Sigorta Acentesi','Emlak Ofisi','Noter'],
  'egitim':          ['Dershane & Etüt Merkezi','Dil Okulu','Müzik Kursu','Sürücü Kursu','Bilgisayar Kursu','Anaokulu & Kreş'],
  'eglence-kultur':  ['Sinema','Müze & Sanat Galerisi','Oyun Merkezi','Düğün & Organizasyon Salonu','Bowling & Bilardo','Escape Room'],
  'konaklama':       ['Otel','Pansiyon & Hostel','Apart Otel'],
  'evcil-hayvan':    ['Veteriner','Pet Shop','Hayvan Bakımevi'],
  'ulasim-arac':     ['Oto Galeri','Araç Kiralama','Oto Yıkama','Lastikçi','Oto Ekspertiz'],
};
...
function main() {
  ...
  console.log('📋 Sütunlar:', cols.join(', '));

  // Mevcut unique constraint'leri göster
  const indexes = qdb.prepare("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='jobs'").all();
  console.log('\n🔒 Mevcut index\'ler:');
  indexes.forEach(idx => console.log(`   ${idx.name}: ${idx.sql || '(otomatik)'}`));

  // Mevcut jobları query bazlı key ile al
  const existingByQuery = new Set();
  qdb.prepare("SELECT il_slug, ilce_slug, query FROM jobs").all().forEach(r => {
    existingByQuery.add(`${r.il_slug}|${r.ilce_slug}|${(r.query||'').toLowerCase().trim()}`);
  });

  console.log(`\n📦 Mevcut toplam job: ${existingByQuery.size}`);

  // Eksik jobları hesapla
  const toInsert = [];
  ...
  const totalExpected = ILLER.reduce((s,x) => s + x.ilceler.length, 0) *
    Object.values(KATEGORI_ARAMALAR).reduce((s,v) => s + v.length, 0);

  console.log(`\n📊 ÖZET`);
  console.log(`   Beklenen toplam job : ${totalExpected}`);
  console.log(`   Mevcut             : ${existingByQuery.size}`);
  console.log(`   Eklenecek eksik    : ${toInsert.length}`);

  if (!commit) {
    console.log(`\n⚠️  DRY RUN — Eklemek için: node scraper/fix-queue.cjs --commit\n`);
    qdb.close();
    return;
  }

  if (toInsert.length === 0) {
    console.log('\n✅ Eksik job yok, her şey tamam!');
    qdb.close();
    return;
  }

  console.log(`\n💾 ${toInsert.length} eksik job ekleniyor...`);

  // UNIQUE constraint bypass: tablo kopyala, constraint olmadan yeniden oluştur
  qdb.exec(`
    BEGIN;
    ...
    -- Constraint OLMADAN yeniden oluştur (query bazlı duplicate kontrolü scriptte yapılıyor)
    CREATE TABLE jobs (
  ...
  console.log('✅ UNIQUE constraint kaldırıldı');

  // Şimdi eksik jobları ekle
  const insertStmt = qdb.prepare(
    `INSERT INTO jobs (il, ilce, il_slug, ilce_slug, query, kategori, status, priority)
     VALUES (?, ?, ?, ?, ?, ?, 'pending', 0)`
  );

  const insertMany = qdb.transaction((rows) => {
    for (const r of rows) {
      insertStmt.run(r.il, r.ilce, r.il_slug, r.ilce_slug, r.query, r.kategori);
    }
  });

  insertMany(toInsert);

  const newTotal = qdb.prepare("SELECT COUNT(*) as c FROM jobs").get().c;
  const newPending = qdb.prepare("SELECT COUNT(*) as c FROM jobs WHERE status='pending'").get().c;

  console.log(`\n✅ Tamamlandı!`);
  console.log(`   Eklenen job   : ${toInsert.length}`);
  console.log(`   Toplam job    : ${newTotal}`);
  console.log(`   Pending job   : ${newPending}`);
