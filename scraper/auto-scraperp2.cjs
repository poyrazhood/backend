...
// Aktif profil (CLI'dan --profile ile değişir, varsayılan: openclaw)
let ACTIVE_PROFILE = 'openclaw';

function ocNavigate(url) {
  const r = spawnSync('openclaw', ['browser', '--browser-profile', ACTIVE_PROFILE, 'navigate', url],
...
function ocSnapshot() {
  const r = spawnSync('openclaw', ['browser', '--browser-profile', ACTIVE_PROFILE, 'snapshot'],
...
async function main() {
  ...
  // Profil seç — varsayılan: openclaw
