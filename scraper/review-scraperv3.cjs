...
async function scrollFeed(page) {
  ...
  for (let s = 0; s < CFG.MAX_SCROLL; s++) {
    ...
    if (h === -1) { await sleep(800); continue; }
    await sleep(CFG.SCROLL_WAIT);
    if (h === lastHeight) {
      stall++;
      if (stall >= 3) break; // scrollHeight 3 kez değişmediyse feed bitti
    } else {
      stall = 0;
    }
    lastHeight = h;
...
async function saveReviews(page, business, pageUrl, parseSrc) {
  // "Devamını oku" butonlarını aç — jsaction için dispatchEvent gerekli
  const moreBtns = page.locator('button.w8nwRe, button[jsname="MyVnn"]');
  const btnCount = await moreBtns.count().catch(() => 0);
  for (let i = 0; i < btnCount; i++) {
    await moreBtns.nth(i).dispatchEvent('click').catch(() => {});
  }
...
async function scrapeOne(page, business) {
  ...
  // Direkt En yeni sıralamasına geç (pass1 yok)
  let totalSaved = 0;
  ...
  if (await sortBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    if (await newestOpt.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const pass = await saveReviews(page, business, pageUrl, parseSrc);
      totalSaved = pass.saved;
    }
  }
