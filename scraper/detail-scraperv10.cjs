...
// Orijinal il adlarını sakla (normalize → orijinal mapping)
const IL_ORIGINAL = new Map(); // ilNorm → orijinal il adı
const ILCE_TO_IL  = new Map(); // ilçeNorm → { il: orijinal, ilNorm }

(function buildOriginalMaps() {
  const list = Array.isArray(ilIlceData)
    ? ilIlceData
    : (ilIlceData.turkiye || ilIlceData.iller || Object.values(ilIlceData)[0]);
  for (const entry of list) {
    const ilOrig = entry.il || entry.name || entry.sehir;
    const ilNorm = normTR(ilOrig);
    IL_ORIGINAL.set(ilNorm, ilOrig);
    for (const ilce of (entry.ilceler || entry.districts || [])) {
      ILCE_TO_IL.set(normTR(ilce), { il: ilOrig, ilNorm, ilce });
    }
  }
})();

/**
 * Scraped adresten gerçek il ve ilçeyi tespit eder.
 * Tüm 81 il + ilçeye karşı tarar.
 * @returns {{ city: string|null, district: string|null, method: string }}
 */
function detectCityFromAddress(address) {
  if (!address) return { city: null, district: null, method: 'no_address' };

  const addrNorm = normTR(address);

  // 1. Önce il adlarını tara (uzun eşleşmeler önce — "afyonkarahisar" gibi)
  const ilMatches = [];
  for (const [ilNorm, ilOrig] of IL_ORIGINAL) {
    if (addrNorm.includes(ilNorm)) {
      ilMatches.push({ ilNorm, ilOrig, len: ilNorm.length });
    }
  }
  // En uzun (en spesifik) il adını al
  if (ilMatches.length) {
    ilMatches.sort((a, b) => b.len - a.len);
    const best = ilMatches[0];
    // İlçeyi de tespit et
    const keywords = IL_KEYWORDS.get(best.ilNorm);
    let district = null;
    if (keywords) {
      for (const kw of keywords) {
        if (kw !== best.ilNorm && kw.length > 2 && addrNorm.includes(kw)) {
          // ILCE_TO_IL'den orijinal ilçe adını bul
          const ilceInfo = ILCE_TO_IL.get(kw);
          if (ilceInfo && ilceInfo.ilNorm === best.ilNorm) {
            district = ilceInfo.ilce;
            break;
          }
        }
      }
    }
    return { city: best.ilOrig, district, method: 'il_match' };
  }

  // 2. İl bulunamazsa ilçe adından ili tespit et
  // Önce uzun ilçe adlarını dene
  const ilceMatches = [];
  for (const [ilceNorm, info] of ILCE_TO_IL) {
    if (ilceNorm.length > 3 && addrNorm.includes(ilceNorm)) {
      ilceMatches.push({ ilceNorm, ...info, len: ilceNorm.length });
    }
  }
  if (ilceMatches.length) {
    ilceMatches.sort((a, b) => b.len - a.len);
    const best = ilceMatches[0];
    return { city: best.il, district: best.ilce, method: 'ilce_match' };
  }

  return { city: null, district: null, method: 'not_detected' };
}
 ...
 */
function checkAddressMatch(address, city) {
  if (!address || !city) return { match: false, matchedKeyword: null };
 ...
 */
function logAddressMismatch(business, scrapedAddress, detectedCity, detectedDistrict) {
  const entry = {
    ts              : new Date().toISOString(),
    id              : business.id,
    name            : business.name,
    oldCity         : business.city,
    oldDistrict     : business.district,
    detectedCity,
    detectedDistrict,
...
async function parseDetail(page) {
  return await page.evaluate(() => {
    ...
    const ratingEl = document.querySelector('.F7nice span[aria-hidden="true"], [aria-label*="yıldız"]');
    if (ratingEl) {
      const raw = (ratingEl.getAttribute('aria-label') || ratingEl.textContent || '').replace(',', '.');
      const m = raw.match(/[\d.]+/);
      if (m) d.averageRating = parseFloat(m[0]);
    }
    const reviewCountEl = document.querySelector('button[aria-label*="yorum"], [data-item-id="review-summary"]');
    if (reviewCountEl) {
      const raw = reviewCountEl.getAttribute('aria-label') || reviewCountEl.textContent || '';
      const m = raw.replace(/\./g, '').match(/\d+/);
      if (m) d.totalReviews = parseInt(m[0]);
    }

    // coverPhoto scrapeOne'da ayrıca çekiliyor
...
async function scrapeOne(page, business) {
  ...
  // SPA render bekle — feed (arama listesi) veya place elementi gelene kadar
  await page.waitForSelector('[role="feed"], button.hh2c6, [data-item-id="address"]', { timeout: 8000 }).catch(() => {});
  ...
  // Kapak fotoğrafı — Hakkında'ya geçmeden önce çek
  // Playwright locator + src attribute ile doğrudan oku
  let coverPhoto = null;
  let businessPhotos = [];
  try {
    ...
    for (let i = 0; i < Math.min(imgCount, 20); i++) {
      ...
      if (src && src.includes('googleusercontent')) {
        // Profil/avatar fotoğraflarını atla (w36-h36, rp-mo, br100 içerenler)
        if (src.includes('rp-mo') || src.includes('br100') || src.includes('w36-h36')) continue;
        // URL'yi normalize et — boyut parametresini standartlaştır
        const normalized = src.replace(/=w\d+-h\d+-k-no/, '=w800-h600-k-no')
                              .replace(/=w\d+-h\d+-p-k-no/, '=w800-h600-k-no')
                              .replace(/=s\d+/, '=w800-h600-k-no');
        if (!candidates.includes(normalized)) candidates.push(normalized);
      }
    }
    if (candidates.length) {
      coverPhoto = candidates[0]; // İlk = en büyük/öne çıkan
      businessPhotos = candidates.slice(0, 8); // Galeri için ilk 8
    }
  } catch {}
  if (process.env.DEBUG) console.log('  📸 coverPhoto:', coverPhoto ? coverPhoto.substring(0, 60) : 'YOK', '| photos:', businessPhotos.length);
  ...
  if (detail.address && detail.address.length > (business.address?.length || 0)) {
    const { match } = checkAddressMatch(detail.address, business.city);
    if (match) {
      // Adres zaten doğru ile ait — sadece adresi güncelle
      update.address = detail.address;
    } else {
      // Adres farklı bir ile ait — adresten gerçek il/ilçeyi tespit et
      const { city: detectedCity, district: detectedDistrict, method } = detectCityFromAddress(detail.address);

      if (detectedCity) {
        // Gerçek il/ilçe bulundu — adres + city + district güncelle
        update.address  = detail.address;
        update.city     = detectedCity;
        if (detectedDistrict) update.district = detectedDistrict;
        console.warn(`⚠️  Adres düzeltildi | ${business.name} | ${business.city}→${detectedCity}${detectedDistrict ? '/'+detectedDistrict : ''} | ${detail.address}`);
        logAddressMismatch(business, detail.address, detectedCity, detectedDistrict);
      } else {
        // İl tespit edilemedi — adresi kaydet ama city/district'e dokunma
        update.address = detail.address;
        console.warn(`⚠️  İl tespit edilemedi — adres kaydedildi | ${business.name} | city: ${business.city} | adres: ${detail.address}`);
        logAddressMismatch(business, detail.address, null, null);
      }
    }
  }
  ...
  if (detail.averageRating && !business.averageRating)                                  update.averageRating = detail.averageRating;
  if (detail.totalReviews  && !business.totalReviews)                                   update.totalReviews  = detail.totalReviews;
  if (coverPhoto) {
    const existingAttrs = (business.attributes && typeof business.attributes === 'object')
      ? business.attributes : {};
    update.attributes = {
      ...existingAttrs,
      coverPhoto,
    ...
    console.log('  📸 coverPhoto bulundu, galeri:', businessPhotos.length, 'fotoğraf');
  } else {
    console.log('  📸 coverPhoto: YOK');
  }
  ...
  // Hakkında — update.attributes varsa onu temel al, yoksa DB'deki mevcut değerleri kullan
  const about = await parseAbout(page);
  if (about) {
    const baseAttrs = update.attributes
      || ((business.attributes && typeof business.attributes === 'object') ? business.attributes : {});
    await prisma.business.update({
      where: { id: business.id },
      data:  { attributes: { ...baseAttrs, about, scrapedAt: new Date().toISOString() } },
    });
  }
...
async function runWorker(wIdx, queue, state) {
  const browser = await chromium.launch({
    ...
    args: [
      ...
      '--disable-gpu',
      '--disable-extensions',
      '--blink-settings=imagesEnabled=false',   // --disable-images Playwright'ta bu şekilde
