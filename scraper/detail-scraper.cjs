...
async function parseAbout(page) {
  // Tüm sekmeleri tara, "Hakkında" olanı JS ile tıkla
  const clicked = await page.evaluate(() => {
    ...
    const tab = candidates.find(el => {
      ...
      return t === 'Hakkında' || t === 'About' || el.getAttribute('aria-label')?.includes('Hakkında');
    });
    if (!tab) return false;
    tab.click();
    return true;
  ...
  await sleep(CFG.ACTION_WAIT + 200);
  ...
  await page.evaluate(() => {
    ...
    const tab = candidates.find(el => {
      const t = el.textContent?.trim();
      return t === 'Genel Bakış' || t === 'Overview' || el.getAttribute('aria-label')?.includes('Genel Bakış');
    });
    if (tab) tab.click();
  });
