...
function ocNavigate(url) {
  // Sıra önemli: --browser-profile ve --url önce, komut sonra
  const args = ['browser', '--browser-profile', ACTIVE_PROFILE];
  if (GATEWAY_URL) args.push('--url', GATEWAY_URL);
  args.push('navigate', url);
  const r = spawnSync('openclaw', args, { encoding: 'utf8', shell: true, timeout: 15000 });
...
function ocSnapshot() {
  ...
  if (GATEWAY_URL) args.push('--url', GATEWAY_URL);
  args.push('snapshot');
  const r = spawnSync('openclaw', args, { encoding: 'utf8', shell: true, maxBuffer: 10 * 1024 * 1024, timeout: 20000 });
  if (r.error) throw new Error(`Snapshot hatası: ${r.error.message}`);
  return r.stdout || '';
}

function ocEvaluate(fn) {
  const args = ['browser', '--browser-profile', ACTIVE_PROFILE];
  if (GATEWAY_URL) args.push('--url', GATEWAY_URL);
  args.push('evaluate', '--fn', fn);
  const r = spawnSync('openclaw', args, { encoding: 'utf8', shell: true, timeout: 10000 });
  return r.stdout || '';
}

// Google Maps sonuç listesini aşağı kaydır
function ocScrollResults() {
  // Google Maps'te sonuç listesi [role=feed] div'i içinde scroll olur
  ocEvaluate(`() => {
    const feed = document.querySelector('[role="feed"]');
    if (feed) { feed.scrollTop += 1500; return true; }
    // Fallback: ana scroll alanı
    const main = document.querySelector('#QA0Szd');
    if (main) { main.scrollTop += 1500; return true; }
...
// ─── ADRES FİLTRESİ ───────────────────────────────────────────────────────────
// Türkiye 81 il listesi (slug + orijinal isim)
const TURKEY_ILLS = [
...
function isAddressFromWrongCity(address, jobIl) {
  if (!address) return false; // adres yoksa filtreleme

  const addrNorm = toSlugTR(address);
  const jobIlNorm = toSlugTR(jobIl);

  // Adreste job'un ilinin adı geçiyorsa zaten doğru şehir
  if (addrNorm.includes(jobIlNorm)) return false;

  // Adreste başka bir ilin adı geçiyorsa yanlış şehir
  for (const il of TURKEY_ILLS) {
    if (il === jobIlNorm) continue; // kendi ilini atla
    if (addrNorm.includes(il)) return true; // başka il bulundu → yanlış
  }

  return false; // sadece sokak/mahalle varsa geçir
}
...
async function importWithCategory(businesses, job, category) {
  ...
  for (const b of businesses) {
    try {
      // Adres başka bir ilde mi? Atla
      if (isAddressFromWrongCity(b.address, job.il)) {
        skipped++;
        continue;
      }
...
async function runJob(job, qdb, opts = {}) {
  ...
  try {
    ...
    if (businesses.length > 0 && !isDirectPlacePage(snapshot)) {
      ...
      const MAX_SCROLLS = 5;   // max 5 scroll = ~100 işletme
      const SCROLL_WAIT = 2000; // her scroll sonrası bekleme (ms)
      ...
      for (let scroll = 0; scroll < MAX_SCROLLS; scroll++) {
        ...
        // Yeni işletme gelmiyorsa liste sona erdi
        if (added === 0) break;
