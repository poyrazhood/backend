...
const TIMEOUT_NAVIGATE = 30000;
const TIMEOUT_SNAPSHOT = 45000;
const TIMEOUT_EVALUATE = 20000;
const MAX_RETRIES      = 3;
...
async function importWithCategory(businesses, job, category) {
  ...
  for (const b of businesses) {
    try {
      if (isAddressFromWrongCity(b.address, job.il)) { skipped++; continue; }
      ...
      if (b.googlePlaceId) {
        ...
        if (existing) {
          await prisma.business.update({
            ...
            data: { averageRating: data.averageRating, totalReviews: data.totalReviews, attributes: attrs, categoryId: category.id },
          });
          updated++;
        } else {
