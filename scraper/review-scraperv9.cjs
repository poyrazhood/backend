...
// Prisma bağlantı havuzu — worker başına max 3 bağlantı
prisma.$connect();
...
const CFG = {
  NAVIGATE_WAIT     : 2000,  // Biraz artırıldı — CPU spike azalır
  ACTION_WAIT       : 1000,
  SCROLL_WAIT       : 800,
  MORE_BTN_WAIT     : 300,
  MIN_DELAY         : 500,   // Artırıldı — CPU dinlenir
  MAX_DELAY         : 900,
  HEADLESS          : true,
  MAX_SCROLL        : 8,
};
...
async function navigateToPlace(page, business) {
  ...
  if (!isList) {
    // URL + DOM kontrolü — place sayfasında sekme veya adres butonu olur
  ...
  const results = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('.Nv2PK, [role="article"]')).map(card => {
      ...
      const chijM  = href.match(/!1s(ChIJ[^!&"]+)/);
      const hexM   = href.match(/!1s(0x[^!:]+:[^!&"]+)/);
      return {
        name    : (nameEl?.textContent?.trim() || '').toLowerCase(),
        href,
        lat     : coordM ? parseFloat(coordM[1]) : null,
        lng     : coordM ? parseFloat(coordM[2]) : null,
        placeId : chijM  ? decodeURIComponent(chijM[1]) : (hexM ? hexM[1] : null),
      };
    }).filter(r => r.href);
  });
  ...
  // Öncelik 1 — PlaceID tam eşleşme
  if (business.googlePlaceId) {
    const byId = results.find(r => r.placeId === business.googlePlaceId);
    if (byId) { targetHref = byId.href; matchMethod = 'placeId'; }
  }

  // Öncelik 2 — Koordinat (~500m tolerans)
  if (!targetHref && business.latitude && business.longitude) {
  ...
  // Öncelik 3 — İsim tam eşleşme
  if (!targetHref) {
    const byName = results.find(r => bizWords.every(w => r.name.includes(w)));
    if (byName) { targetHref = byName.href; matchMethod = 'isim (tam)'; }
  }

  // Öncelik 4 — Tek sonuç + isim benzerliği
  if (!targetHref && results.length === 1 && bizWords.some(w => results[0].name.includes(w))) {
    targetHref = results[0].href; matchMethod = 'tek sonuç + isim';
  }

  if (!targetHref) {
    console.warn(`  ⚠️  Eşleşme bulunamadı: ${business.name} (${results.length} sonuç)`);
    return false;
  }

  console.log(`  🎯 ${matchMethod} — ${business.name}`);

  await page.goto(targetHref, { waitUntil: 'domcontentloaded', timeout: 25000 });
  await sleep(CFG.NAVIGATE_WAIT);

  // Reload — SPA sekmelerin tam yüklenmesi için
...
async function parseReviews(page, parseSrc) {
  return await page.evaluate((src) => {
    ...
    // Sadece en üst seviye kart elementlerini al
    // [data-review-id] kart içindeki butonlarda da var — parent kontrolü ile filtrele
    let cards = Array.from(document.querySelectorAll('[data-review-id]'))
      .filter(el => !el.parentElement?.closest('[data-review-id]'));
    // Fallback: .jftiEf dene
    if (!cards.length) cards = Array.from(document.querySelectorAll('.jftiEf'));

    for (const card of cards) {
      // ── Yazar ──────────────────────────────────────────────────────────────
      // Önce DOM'dan dene, yoksa aria-label'dan parse et
      // aria-label formatı: "X adlı kullanıcının fotoğrafı"
      let authorName = card.querySelector('.d4r55, [class*="d4r55"], .WNxzHc a')?.textContent?.trim();
      if (!authorName || authorName.length < 2) {
        const photoLabel = card.querySelector('[aria-label*="adlı kullanıcının fotoğrafı"]')?.getAttribute('aria-label') || '';
        const m = photoLabel.match(/^(.+?) adlı kullanıcının fotoğrafı/);
      ...
      // ── Puan ───────────────────────────────────────────────────────────────
      // aria-label formatı: "5 yıldız" — role="img" şartı kaldırıldı
      const ratingEl = card.querySelector('[aria-label*="yıldız"], [aria-label*="star"]');
      const ratingMatch = (ratingEl?.getAttribute('aria-label') || '').match(/(\d+)\s*(yıldız|star)/i);
      ...
      // ── Yorum fotoğrafları ─────────────────────────────────────────────────
      // Fotoğraflar <img> değil, background-image CSS style içinde
      const photoEls = card.querySelectorAll('button.Tya61d, [class*="Tya61d"]');
      const photos = Array.from(photoEls).map(btn => {
...
async function scrollFeed(page) {
  ...
  for (let s = 0; s < CFG.MAX_SCROLL; s++) {
    const result = await page.evaluate(() => {
      ...
      if (!f) return { scrolled: false, cardCount: 0 };
      f.scrollTop = f.scrollHeight;
      const cards = f.querySelectorAll('[data-review-id]').length;
      return { scrolled: true, cardCount: cards };
    });
    if (!result.scrolled) {
      await sleep(800);
      continue;
    }
    // Yeni kart gelmiyorsa dur — gereksiz scroll yapma
    if (s > 2 && result.cardCount === lastCount) break;
    lastCount = result.cardCount;
...
async function saveReviews(page, business, pageUrl, parseSrc) {
  // "Devamını oku" — sadece yorum kartı içindeki genişletme butonları
  // button.w8nwRe / jsname="MyVnn" → Google Maps'in kesin class'ı, geniş selector değil
  const moreBtns = page.locator('button.w8nwRe, button[jsname="MyVnn"]');
  const btnCount = await moreBtns.count().catch(() => 0);
  for (let i = 0; i < btnCount; i++) {
    const btn = moreBtns.nth(i);
    const visible = await btn.isVisible({ timeout: 0 }).catch(() => false);
    if (visible) await btn.dispatchEvent('click').catch(() => {});
  }
  if (btnCount > 0) await sleep(CFG.MORE_BTN_WAIT);
...
async function scrapeOne(page, business) {
  ...
  if (!onPlace) return { status: 'not_found', reviewsSaved: 0 };
  ...
  // ── İşletme bilgilerini ana sekmeden çek (Genel Bakış) ────────────────────
  const bizInfo = await page.evaluate(() => {
    const info = {};

    // Genel puan — aria-label: "4,5 yıldız" veya "4.5 stars"
    const ratingEl = document.querySelector(
      'div[jsaction*="pane.rating"] span[aria-hidden="true"], ' +
      '.F7nice span[aria-hidden="true"], ' +
      'span.ceNzKf, [class*="ceNzKf"]'
    );
    if (ratingEl) {
      const txt = ratingEl.textContent?.trim().replace(',', '.');
      const n = parseFloat(txt);
      if (!isNaN(n) && n >= 1 && n <= 5) info.rating = n;
    }

    // Toplam yorum sayısı — "1.234 yorum" veya "(1,234)"
    for (const el of document.querySelectorAll(
      'button[jsaction*="review"] span, .HHrUdb, [class*="HHrUdb"], ' +
      'span[aria-label*="yorum"], button[aria-label*="yorum"]'
    )) {
      const txt = el.textContent?.trim() || '';
      const m = txt.match(/([\d.,]+)\s*yorum/) || txt.match(/\(([\d.,]+)\)/);
      if (m) {
        const n = parseInt(m[1].replace(/[.,]/g, ''));
        if (!isNaN(n) && n > 0) { info.reviewCount = n; break; }
      }
    }

    // İşletme fotoğrafları — kapak + galeri
    const photos = [];
    // Kapak fotoğrafı
    for (const img of document.querySelectorAll(
      'button[jsaction*="photo"] img, .ZKCDEc img, [class*="ZKCDEc"] img, ' +
      '.RZ66Rb img, [class*="RZ66Rb"] img'
    )) {
      const src = img?.src || img?.getAttribute('data-src') || '';
      if (src.startsWith('https://lh') || src.startsWith('https://streetviewpixels')) {
        // Yüksek çözünürlük için =w800-h500 parametresi ekle
        const clean = src.replace(/=w\d+-h\d+.*$/, '=w800-h600');
  ...
  // İşletme rating + reviewCount + photos → business tablosuna kaydet
  const bizUpdate = {};
  if (bizInfo.rating      && !business.averageRating) bizUpdate.averageRating = bizInfo.rating;
  if (bizInfo.reviewCount && !business.totalReviews)  bizUpdate.totalReviews  = bizInfo.reviewCount;
  if (bizInfo.photos?.length) {
    const existing = business.attributes || {};
    const existingPhotos = existing.photos || [];
    const merged = [...new Set([...existingPhotos, ...bizInfo.photos])].slice(0, 20);
    if (merged.length > existingPhotos.length) {
      bizUpdate.attributes = { ...existing, photos: merged };
    }
  }
  if (Object.keys(bizUpdate).length) {
    await prisma.business.update({ where: { id: business.id }, data: bizUpdate }).catch(() => {});
  }
  ...
  // Geçiş 1: En faydalı — scroll olmadan görünenleri kaydet
  ...
  if (await sortBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    if (await newestOpt.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      // Eski feed kaybolana kadar bekle, sonra yenisini bekle
      await page.waitForSelector('[role="feed"]', { state: 'detached', timeout: 3000 }).catch(() => {});
...
async function runWorker(wIdx, queue, state) {
  const browser = await chromium.launch({
    ...
    args: [
      ...
      '--lang=tr-TR',
      '--mute-audio',
      '--disable-gpu',                    // GPU kullanma — CPU'ya yük azalır
      '--disable-extensions',             // Extension yok
      '--disable-background-networking',  // Arka plan ağ isteği yok
      '--disable-background-timer-throttling',
      '--disable-renderer-backgrounding',
      '--disable-backgrounding-occluded-windows',
      '--disable-ipc-flooding-protection',
      '--memory-pressure-off',            // RAM baskısı yönetimi
      '--js-flags=--max-old-space-size=256', // Her tab max 256MB
    ],
  });
  const ctx = await browser.newContext({
    locale: 'tr-TR',
    userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36',
    viewport: { width: 800, height: 600 }, // Küçük viewport — daha az render
  });
  ...
  try {
    while (true) {
      ...
      const biz = await prisma.business.findUnique({
        ...
        select: { id: true, name: true, city: true, district: true, googlePlaceId: true, latitude: true, longitude: true, averageRating: true, totalReviews: true, attributes: true },
      });
      ...
      // Her 30 işletmede bir sayfayı sıfırla — DOM birikimini temizle
      const workerProcessed = state.processed;
      if (workerProcessed % 15 === 0) {
        await page.goto('about:blank', { timeout: 5000 }).catch(() => {});
        await sleep(500);
        // Node.js GC'yi tetikle
...
async function run(opts = {}) {
  const { workerCount = 5, il = null, reset = false, offset = 0, limit = null } = opts;
...
async function cmdTest(args) {
  ...
  const biz = id
    ? await prisma.business.findUnique({ where: { id }, select: { id: true, name: true, city: true, district: true, googlePlaceId: true, latitude: true, longitude: true, averageRating: true, totalReviews: true, attributes: true } })
    : await prisma.business.findFirst({ where: { isActive: true, isDeleted: false }, select: { id: true, name: true, city: true, district: true, googlePlaceId: true, latitude: true, longitude: true, averageRating: true, totalReviews: true, attributes: true } });
...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'run':
      await run({
        workerCount : parseInt(get('--workers') || '5'),
        il          : get('--il'),
        reset       : has('--reset'),
