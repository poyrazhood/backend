...
function renderDashboard(state) {
  const { il, workerCount, totalPending, startPending, processed, added, errors, start, workerStatus } = state;
  const elapsed = (Date.now() - start) / 1000 / 60;
  const rate = elapsed > 0.1 ? (processed / elapsed).toFixed(1) : '...';
  const remaining = startPending - processed;
  const eta = rate > 0 ? Math.ceil(remaining / parseFloat(rate)) : '?';
  const pct = startPending > 0 ? ((processed / startPending) * 100).toFixed(1) : '0.0';
  const barLen = 30;
  const filled = Math.round(processed / startPending * barLen) || 0;
  const bar = '█'.repeat(filled) + '░'.repeat(barLen - filled);

  // Terminali temizle (sadece dashboard bloğunu yeniden çiz)
  process.stdout.write('\x1b[2J\x1b[H'); // ekranı temizle

  const lines = [
    `╔══════════════════════════════════════════════════╗`,
    `║         🦞 TECRUBELERIM SCRAPER DASHBOARD        ║`,
    `╠══════════════════════════════════════════════════╣`,
    `║  İl filtresi : ${(il || 'Tümü').padEnd(33)}║`,
    `║  Worker      : ${String(workerCount).padEnd(33)}║`,
    `╠══════════════════════════════════════════════════╣`,
    `║  İlerleme    : [${bar}] ${pct}%  ║`,
    `║  İşlenen     : ${String(processed).padEnd(10)} / ${String(startPending).padEnd(20)}║`,
    `║  Yeni işletme: ${String(added).padEnd(33)}║`,
    `║  Hata        : ${String(errors).padEnd(33)}║`,
    `╠══════════════════════════════════════════════════╣`,
    `║  Hız         : ${(rate + ' job/dk').padEnd(33)}║`,
...
async function run(opts = {}) {
  ...
  const startPending = queue.pending(il, ilce);

  const state = {
    il, workerCount, startPending,
    processed: 0, added: 0, errors: 0,
    start: Date.now(),
    workerStatus: {},
  };

  for (let i = 1; i <= workerCount; i++) state.workerStatus[i] = 'bekliyor...';

  // Dashboard'u her 2 saniyede bir yenile
  const dashInterval = setInterval(() => renderDashboard(state), 2000);
  renderDashboard(state);

  // Worker'ları state'i güncelleyecek şekilde sarmala
  async function workerWithStats(wIdx) {
    const { il, ilce } = opts;
    let browser;
    ...
    try {
      while (true) {
        ...
        const shortQuery = job.query.replace(/^[^ ]+ [^ ]+ /, ''); // "İl İlçe Kategori" → "Kategori"
        state.workerStatus[wIdx] = `🔍 ${shortQuery} (${job.ilce})`;
  ...
  const workers = Array.from({ length: workerCount }, (_, i) => workerWithStats(i + 1));
  await Promise.all(workers);

  clearInterval(dashInterval);
  renderDashboard(state); // son render
