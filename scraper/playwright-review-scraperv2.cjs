...
async function parseBusinessDetailFromPage(page) {
  return await page.evaluate(() => {
    ...
    for (const sel of addressSelectors) {
      ...
      if (label.includes(':')) { detail.address = label.split(':').slice(1).join(':').trim(); break; }
      const txt = el.textContent?.trim();
      if (txt && txt.length > 5) { detail.address = txt; break; }
    }
    if (!detail.address) {
      const addrEl = document.querySelector('.rogA2c, .Io6YTe');
      if (addrEl) detail.address = addrEl.textContent?.trim() || null;
    }
    ...
    // Google Place ID — ChIJ base64 formatını URL'den çek
    // /maps/place/.../@lat,lng,17z/data=...!1sChIJ... formatında
    const placeChIJ = url.match(/!1s(ChIJ[^!&"]+)/);
    if (placeChIJ) {
      detail.googlePlaceId = decodeURIComponent(placeChIJ[1]);
    } else {
      // Fallback: hex format (eski kayıtlar için)
      const placeHex = url.match(/!1s(0x[^!:]+:[^!&"]+)/);
      if (placeHex) detail.googlePlaceId = placeHex[1];
    }
...
function buildSearchUrl(business) {
  // googlePlaceId sadece ChIJ... (base64) formatında çalışır
  // 0x... (hex) format Google Maps place URL'de çalışmaz → arama kullan
  if (business.googlePlaceId && business.googlePlaceId.startsWith('ChIJ')) {
...
async function scrapeBusiness(page, business, opts = {}) {
  ...
  if (isSearchList) {
    ...
    // Google Maps arama sonuçlarını DOM'dan oku
    const results = await page.evaluate(() => {
      const items = [];
      // .Nv2PK — Google Maps'te her işletme kartının wrapper class'ı
      const cards = document.querySelectorAll('.Nv2PK, [role="article"]');
    ...
    for (const r of results) {
      const rName = r.name.toLowerCase().normalize('NFC');
      if (
        rName === bizNameNorm ||
        bizWords.every(w => rName.includes(w)) ||
        bizWords.some(w => rName.includes(w))
      ) {
        targetHref = r.href;
        break;
      }
    ...
    // Bulunan href'e git (tıklama yerine navigate — daha güvenilir)
    const gotoHref = targetHref || (results.length > 0 ? results[0].href : null);
    if (gotoHref) {
      await page.goto(gotoHref, { waitUntil: 'domcontentloaded', timeout: 25000 });
      // URL'nin /maps/place/ içermesini bekle
  ...
  if (detail.address       && (!business.address || business.address.trim() === ''))  detailUpdate.address = detail.address;
  ...
  // Yorumlar sekmesi — farklı selector'lar dene
...
async function cmdTest(args) {
  ...
  try {
    ...
    console.log('\n📋 Şu anki URL:', page.url());
    console.log('\n⏸️  Browser açık bekliyor... Kapatmak için Enter\'a bas.');
    ...
    console.log('\n⏸️  Hata sonrası bekliyor... Enter\'a bas.');
    await new Promise(resolve => {
      process.stdin.setRawMode(true);
      process.stdin.resume();
      process.stdin.once('data', () => { process.stdin.setRawMode(false); resolve(); });
