...
async function navigateToPlace(page, business) {
  ...
  if (!isList) {
    // Zaten place sayfasındayız — reload ile sekmeleri yükle
    if (urlNow.includes('/maps/place/') || urlNow.includes('place_id')) {
...
async function scrapeOne(page, business) {
  ...
  // Arama listesine düştüyse doğru işletmeye geç
  await navigateToPlace(page, business);

  // Her durumda F5 — Google Maps SPA'da sekmeler ancak reload ile tam gelir
  // Reload öncesi URL'yi kaydet
  ...
  // URL kontrolü — reload önceki veya sonraki URL place içeriyorsa devam et
  const urlAfterReload = page.url();
  console.log('  🌐 URL (reload sonrası):', urlAfterReload.substring(0, 100));
  const isOnPlace = urlBeforeReload.includes('/maps/place/') || urlAfterReload.includes('/maps/place/')
    || urlBeforeReload.includes('@') || urlAfterReload.includes('@');
  if (!isOnPlace) return { status: 'not_found' };
