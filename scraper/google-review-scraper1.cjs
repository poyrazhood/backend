...
function parseBusinessDetail(snapshot) {
  ...
  // Telefon — gerçek format (debug'dan doğrulandı):
  //   button "Telefon: 0507 595 53 59" [ref=e175]
  //   /url: tel:05075955359
  const phonePatterns = [
    /button "Telefon:\s*([0-9][0-9\s]{9,})" \[ref=e\d+\]/,
    /\/url:\s*tel:(\+?[0-9]{10,})/,
  ];
  ...
  // Website — /url: satırından al, wa.me ve google domainlerini hariç tut
  const websiteUrlM = snapshot.match(
    /\/url:\s*(https?:\/\/(?!wa\.me|maps\.google|www\.google|support\.google|goo\.gl|accounts\.google)[^\s"]{5,})/
  );
  if (websiteUrlM) detail.website = websiteUrlM[1];
...
function parseReviews(text) {
  const reviews = [];
  const blockRegex = /^ {8,16}- generic "([^"]+)" \[ref=e\d+\]:/gm;
  const blockStarts = [];
  let m;
...
async function scrapeBusiness(business, opts = {}) {
  ...
  if (sortBtnMatch) {
    // Sıralama butonuna tıkla ve menünün açılmasını bekle
    ocClick(sortBtnMatch[1]);
    await sleep(2000);
    let sortMenuSnap = ocSnapshot();

    // menuitemradio, menuitem, option, listitem — hepsini yakala
    let newestMatch =
      sortMenuSnap.match(/menuitemradio "En yeni[^"]*" \[ref=(e\d+)\]/) ||
      sortMenuSnap.match(/(?:menuitem|option|listitem|radio|generic) "En yeni[^"]*" \[ref=(e\d+)\]/);

    if (!newestMatch) {
      // Menü henüz açılmamış olabilir — yeniden dene
      const retryBtn = sortMenuSnap.match(/button "(?:En alakalı|Sıralama ölçütü)[^"]*" \[ref=(e\d+)\]/);
      if (retryBtn) {
        ocClick(retryBtn[1]);
        await sleep(2000);
        sortMenuSnap = ocSnapshot();
      }
      newestMatch =
        sortMenuSnap.match(/menuitemradio "En yeni[^"]*" \[ref=(e\d+)\]/) ||
        sortMenuSnap.match(/(?:menuitem|option|listitem|radio|generic) "En yeni[^"]*" \[ref=(e\d+)\]/);
    ...
    if (newestMatch) {
      ...
      const menuItems = [...sortMenuSnap.matchAll(/"([^"]{3,40})" \[ref=e\d+\]/g)];
      if (menuItems.length > 0) console.log('  Snap içeriği (ilk 10):', menuItems.slice(0, 10).map(x => x[1]).join(' | '));
    }
  } else {
