...
// AnalyticsTab sonuna radar ekle - </div>\n  )\n}\n\nfunction ReviewsTab satirindan once
const oldEnd = `    </div>
  )
}
...
function AutoServiceManualForm({ businessId }: { businessId: string }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.sertifikalar) body.sertifikalar = form.sertifikalar.split(',').map(s => s.trim()).filter(Boolean)
      if (form.uzmanlikAlanlari) body.uzmanlikAlanlari = form.uzmanlikAlanlari.split(',').map(s => s.trim()).filter(Boolean)
  ...
  return (
    <div className="p-4 rounded-2xl bg-surface-1 border border-white/[0.07]">
      ...
      <div className="space-y-2 mb-3">
        <div>
          <div className="text-[10px] text-white/40 mb-1">Sertifikalar (virgülle ayirin)</div>
        ...
        <div>
          <div className="text-[10px] text-white/40 mb-1">Uzmanlik Alanlari (virgülle ayirin)</div>
...
console.log("Radar ve manuel form eklendi!")